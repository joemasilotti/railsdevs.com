class BusinessQuery
  ...
  def query_and_paginate
    @_records = Business.includes(:user)
    sort_records
  ...
  def sort_records
    @_records.merge!(Business.newest_first)
  end

  def search_query_filter_records
    @_records.merge!(Business.filter_by_search_query(search_query)) unless search_query.empty?
  end
