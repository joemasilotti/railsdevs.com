...
class FieldErrorTagBuilderTest < ActiveSupport::TestCase
  setup do
    @spoofed_instance = Struct.new(:error_message).new(["fake error"])
  end

  test "replace only text color class" do
    html_tag = %(<label class="bg-gray-200 font-medium text-gray-200">Test Label</label>)
    builder = FieldErrorTagBuilder.new(html_tag, @spoofed_instance)
  ...
  test "inserts errors before closing tag" do
    ...
    assert builder.error_field.index(@spoofed_instance.error_message.first) < builder.error_field.index("</label>")
  end
end
