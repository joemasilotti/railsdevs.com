...
##### Creating an API key:
  ...
  1. Click "ADD SECRET KEY", copy it and write it down (we will only be able to see this once)
  1. Under "Redirect URIs" add the omniauth callback for localhost: `http://localhost:3000/auth/docusign/callback`
  1. Write down the integration key next to your App name under "Apps and Integration Keys" (We will be adding both the
     integration key and the secret to our App Credentials next.)

##### Using API key in our App:

  1. Add the `integratino_key` and `secret_key` to development credentials `EDITOR="mate --wait" bin/rails credentials:edit --environment development`
...
Now you can create signatures via Docusign Embedded Signing Service by visiting
http://localhost:3000/hiring_agreement/signature/new and clicking the "Visit Docusign to Electronically Sign the hiring
agreement" button.
...
##### Promoting our API key to Production:
